...
impl ViewerEndpoint {
    ...
    /// Fetch a page from a daemon over the relay, presenting `capability`.
    ///
    ...
    pub async fn fetch(
        ...
        let connection = self.connection_to(daemon, &relay).await?;
        match request_page(&connection, capability.clone(), path).await {
            Err(FetchError::Transport(err)) => {
                ...
                self.connections.lock().unwrap().remove(&daemon);
                let connection = self.connection_to(daemon, &relay).await?;
    ...
    async fn connection_to(
        ...
        let existing = {
            ...
            cache.get(&daemon).cloned()
        };
        if let Some(existing) = existing {
            return Ok(existing);
        }
        let addr = EndpointAddr::from(daemon).with_relay_url(relay.clone());
        let connection = self
            .endpoint
            .connect(addr, ALPN)
            .await
        ...
        self.connections
            .lock()
            .unwrap()
            .insert(daemon, connection.clone());
        Ok(connection)
    }
}
