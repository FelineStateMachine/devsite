# Brokered endpoint access

Brokered access lets a sandboxed worker obtain a narrowly bounded service grant without
giving that worker a person's browser session, a machine credential, or the broker
machine's endpoint key.

## Actors and authority

- The **requester** owns a fresh, temporary Ed25519 endpoint key and creates a signed service
  request. Its public request is safe to hand to a broker; its private key is not.
- The **broker** is an ordinary enrolled machine whose single-use enrollment ticket carried
  the `service_grants:issue` scope. The server stores that scope on the resulting revocable
  machine credential.
- The **control plane** verifies both signatures, enforces the broker scope and account
...
Broker scope is delegated at enrollment instead of inferred from possession of any machine
credential. The dashboard must explicitly include it when minting the registration ticket.
Revoking the resulting machine credential removes grant sessions issued by that broker.
...
The request signature covers a domain separator, schema version, request id, service
keyword, requester endpoint id, and expiry. The command creates new files rather than
overwriting paths and keeps both private by default. Only the request JSON crosses the
sandbox boundary.

The broker first resolves and plans:
...
Resolution searches only owned services and accepted service shares visible to the broker.
...
After user or policy approval, the broker applies the same intent:
...
The broker's persistent `devsite-endpoint.key` signs the request id, selected resource,
requester endpoint, and grant expiry. The server also verifies that the signed request's
keyword still resolves to that resource. It rejects expired requests, grants longer than 15
minutes, duplicate request ids, credentials without broker scope, and resources outside the
broker account's current access. The `dsp_…` token is signed by the control plane over the
broker credential, request id and keyword, resource, endpoint, request expiry, and grant
expiry. Apply requires it, preventing a request file or selected target from being swapped
after approval.

The broker returns only `result.grant` and `result.server`. The requester then runs:
...
grant is broker-issued and bound to the same endpoint, and emits resident NDJSON lifecycle
events. It refuses non-loopback listeners.

## Expiry and revocation

Requests expire within 10 minutes and request ids are single-use at issuance. Grants expire
within 15 minutes. A broker credential has a bounded number of simultaneous active grant
